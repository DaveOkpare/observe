...
def generate_mock_span(span_id: int) -> dict:
    ...
    return {
        ...
        "parent_span_id": f"{span_id - 1:08x}{'0' * 8}"
        if span_id > 0
        else None,  # 16 chars
...
async def direct_copy_from_test(
    batch_size: int = 2048, total_spans: int = 20000, max_concurrent_batches: int = 5
):
    """
    ...
    print(
        f"   Batch time range: {min_batch_time * 1000:.1f}ms - {max_batch_time * 1000:.1f}ms"
    )
...
async def run_batch_size_comparison():
    ...
    for result in results:
        ...
        print(
            f"{result['batch_size']:<12} {result['rps']:<10.0f} "
...
if __name__ == "__main__":
    ...
    print("==================================")
    print(
        "This bypasses the OpenTelemetry Collector and tests pure database performance"
    )
    print()
    ...
    if choice == "1":
        ...
        if confirm.lower() == "y":
            asyncio.run(
                direct_copy_from_test(
                    batch_size=4096, total_spans=50000, max_concurrent_batches=5
                )
            )
    else:
    ...
    print("\n🔍 Check results:")
    print(
        "   PGPASSWORD=postgres psql -h localhost -p 6432 -U postgres -d observability -c 'SELECT COUNT(*) FROM spans;'"
    )
